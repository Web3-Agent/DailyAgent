...
const gemini = async (system, user) => {
  let responseai;
  try {
      // Retrieve API key dynamically from chrome.storage
      ...
      responseai = await fetch("https://api.openai.com/v1/chat/completions", {
          method: "POST",
          headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${apiKey}`, // Using API key from storage
          },
          body: JSON.stringify({
      ...
      console.log("Response from API:", responseai);
      ...
      if (responseai) responseai = null; // Ensuring session is cleared
  }
};
